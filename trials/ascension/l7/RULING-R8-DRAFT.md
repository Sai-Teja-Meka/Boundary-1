...
> and `R7`'s round-2 draft refused to. Under `R2` obligation 4 the corpus binding
> is the human's: computed arithmetic is necessary and never sufficient.
>
> Written at Layer-7 Stage A (`[L6] [ASCEND]`, 2026-08-03) from the measurements
> in `trials/ascension/l7/ATTAINABILITY.md`, every one of which is machine-checked
> by `trials/ascension/l7/t_attainability.py` and `trials/ops/l7/t_l7compose.py`.

---

**Status:** DRAFT — frozen on commit **if** a human appends it.
**Binds (if ratified):** the Layer-7 ascension trials (`trials/ascension/l7/`)
...
- `trials/ascension/l7/ATTAINABILITY.md` — the arithmetic every clause rests on,
  with dated ratification notes above (never inside) its body, its historical
  text unedited and its forward-looking sentences answered rather than rewritten.
- `trials/ascension/l7/t_attainability.py` — every recorded figure re-derived on
  ...
  three `§4.2` blindnesses measured against the **frozen** validator. Its closing
  trial — `trial_no_layer_7_gate_binds_on_anything` — is the one that must be
  **advanced one step** at ratification rather than weakened, in the form
  `t_attainability_b.py`'s was at Layer 6.
- `trials/ops/l7/t_l7compose.py` — both theorems, the two declared readings, the
  lineage ladder, and the re-ingestibility that makes promotion reachable.
- `trials/laws/t_rulings.py` — the eight `§5 L7` constants, which today carry a
  `§5` clause and **no companion ruling** (which is what *"no gate binds"* looks
  like in that registry) and would carry `R8` beside it. `R5`, `R7` and `R8` are
  to be kept **distinct** in every note, as the `R6` session kept `R5` and `R6`
  and the `R7` session kept `R5` and `R7`: `R5` authorizes readings of `R2`'s
  obligations, `R7` the `n/a` law for `AUROC`, and `R8` the substrate, the
  denominators and `§4.2`'s reading.
- `corpora/registry.py`, `trials/ascension/README.md` — dated notes recording
  where *"no Layer-7 gate binds on anything"* stops holding, with no historical
  sentence rewritten.
- The mutation discipline at the current bar, each red on the trial named and
  ...
  the eight constants drifted by one; a smuggled `GATE_SMUGGLED` in
  `ascension/l7` and in `ops/l7`; deleting `R8`; **breaking a twin**, so that a
  mirror pair's two members compose differently; **making the lineage ledger
  decorative**, which is the property clause 5(c) exists to force and which turns
  the promotion ladder red at rung 1; and a rewritten `R8` line, re-run **after**
  the commit, because the append-only prefix walk polices committed history only.
