...
- `l4/` — Layer 4, Consolidation, **Stages A and B; no engine yet.**
  ...
  - Stage B (`t_consolidation.py`): the Q1–Q4 battery, applying the ratified
    ...
    clause 5's conditional arithmetic-skip. **Every trial in it is engine-gated
    and skips until Stage C.** It deliberately does not re-assert Stage A's
    witness or baselines — one fixture, one truth — and its docstring names the
    trial that owns each. Its counterpart `humility/l4/` is green today, which
    is the standing shape of an ASCEND split at the trials/engine boundary.
