...
def trial_the_scorecard_gate_table_is_the_authority_registry():
    ...
    require_equal(sorted(table), [1, 2, 3, 4],
    ...
    for layer in (1, 2, 3):
        for row in card["tiers"][layer]:
            require_equal(row["verdict"], "PASS",
                          "L%d/%s did not pass at the declared reduced scale"
                          % (layer, row["corpus"]))
